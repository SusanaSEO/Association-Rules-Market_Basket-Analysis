...
#Import data set as table. The strip.white eliminate any white lines (wihout the data set has 1,535 rows)
#When using the read.table function the last two elemnets move to the next row, and then creates 1,637 rows when in reality there are
#only 1,499. The col.names function allow us to inform the function about the real number of columns and avoid this issue
#The fill element fill allow having rows with different lenght as it is the case
#
...
#Total of 1,499 rows and 34 columns is the result
...
#Check which type of class is groceries file after importing with read.csv
class(groceries) #data.frame
#
#Explore the first 50 rows of the file
head(groceries, 50)
#
...
#
#ad transaction ID for each row
groceries_clean$transactionID <- seq.int(nrow(groceries_clean))
#
#Doublecheck that all the changes were perfomred
str(groceries_clean)
...
#
#
#Checking again but wihout vegetables
...
#
#
#Checking again but wihout vegetables and increasing the min of items in the itemset to 3
...
#https://rstudio-pubs-static.s3.amazonaws.com/80871_46067c5667e242d9bfb0dc4d212532dc.htmlhttps://rstudio-pubs-static.s3.amazonaws.com/80871_46067c5667e242d9bfb0dc4d212532dc.html
...
#
#Export clean and with just items dataframe into a csv file
